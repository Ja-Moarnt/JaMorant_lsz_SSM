...
import com.JaMorant.SSM.vo.vod.TeacherQueryVo;
...
//    }
    ...
    @ApiOperation("司机登录接口")
    @PostMapping("login")
    public Result login(@RequestBody TeacherLogin teacherLogin) {
        Teacher teacher=teacherService.login(teacherLogin);
        if (teacher!=null) {
            return Result.ok(teacher).message("登录成功");
        }else return Result.fail(null).message("登录失败，请重新输入密码");
    }
...
//        }
    ...
    public Result findPage(@PathVariable long current,
        ...
        if(teacherQueryVo == null) {//查询全部
            ...
            if(!StringUtils.isEmpty(joinDateBegin)) {
                wrapper.ge("create_time",joinDateBegin);
            }
            if(!StringUtils.isEmpty(joinDateEnd)) {
                wrapper.le("create_time",joinDateEnd);
            }
            //调用方法分页查询
    ...
    //5 修改-根据id查询
    @ApiOperation("根据id查询司机（返回Result）")
